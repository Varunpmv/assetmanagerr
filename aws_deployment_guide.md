...
```bash
git clone https://github.com/Varunpmv/assetmanagerr.git
cd assetmanagerr
```

## 4. Configuration (Environment Variables)
Update your `docker-compose.yml` on the server.

### CRITICAL: Update API URLs
In `docker-compose.yml`, update the `VITE_API_URL` to your server's Public IP or Domain:
```yaml
# client service
environment:
...
1. **Nginx Reverse Proxy**: Serve on port 80/443.
2. **SSL**: Use Certbot.
3. **JWT Secret**: Change the default `supersecretjwtkey123`.
